...
^^^^^^^^^^^^^^^^^^^^^^
    ...
    - ``default``
      ...
      .. note:: The prefix ``c`` exists to separate chunk keys from the keys for metadata 
         documents. This is a performance optimization that allows applications to find 
         Zarr metadata documents without traversing a potentially large number of chunk keys.
...
.. note::
   ...
   Inability to delete keys can impair the ability to rename keys, as a rename
